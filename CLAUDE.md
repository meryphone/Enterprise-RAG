...
**Ficheros:** `app/ingestion/parser.py` → `app/ingestion/elements.py` → `app/ingestion/chunker.py` → `app/rag/vector_store.py`
...
- **`nombre_fichero`** (ej: `PR-02.pdf`): embebe el nombre del fichero directamente (con extensión). Mejora el retrieval para queries que mencionan códigos explícitos.
...
**Por qué:** ChromaDB solo admite valores escalares (`str`/`int`/`float`/`bool`) en metadatos, por eso los `None` se mapean a `""` o `-1`. Los metadatos viajan junto al chunk y alimentan el evento `sources` del SSE (chips de fuente en el frontend), el filtrado por scope y los logs de evaluación. El texto que se embebe lleva además los prefijos `tipo_doc · nombre_fichero · titulo · seccion · texto`, pero eso es input del embedding, no metadato almacenado.
...
| `anexo_de` | **administrador** (solo scripts) | Código del documento padre si el fichero es un anexo separado. No se expone en el panel de administración — se asigna desde los scripts de ingesta. |
...
**Toggle:** `ENABLE_QUERY_REWRITING=0` en `.env` desactiva el rewriting y usa la query original para ambos vectores. Útil para depuración o entornos sin cuota de GPT-4o-mini. Por defecto `1` (activado).
...
**Configuración:** `main.py` establece `logging.getLogger("app").setLevel(logging.INFO)` para que todos los loggers bajo `app.*` emitan INFO a través del root handler de uvicorn. No se configura ningún handler adicional — uvicorn ya gestiona el formato y la salida.

**Cobertura por módulo:**

| Módulo | Qué se registra |
|---|---|
| `app/ingestion/parser.py` | Inicio y fin de parse con tiempo (s). Init del singleton de Docling la primera vez. Excepción con traceback si falla. |
| `app/ingestion/elements.py` | Conteo de elementos, tablas e imágenes (logo descartado, sin descripción, fusionadas, standalone). |
| `app/ingestion/chunker.py` | Elementos descartados (sin sección, demasiado cortos) y cuántos pasan al chunker. |
| `app/ingestion/pipeline.py` | Cada uno de los 4 pasos con tiempo elapsed. Excepción con traceback si falla algún paso, indicando cuál. |
| `app/rag/vector_store.py` | Chunks indexados / parents almacenados por colección. Excepción si falla la llamada a OpenAI embeddings o el upsert en ChromaDB. |
| `app/api/admin.py` | Ingesta completada por fichero (children + parents). Error con traceback si falla la ingesta de un fichero. |
...
**`GET /admin/stats`** *(rol admin)*: devuelve el conteo de chunks por colección y totales consultando ChromaDB en tiempo real.

**`POST /admin/ingest`** *(rol admin)*: acepta `multipart/form-data` con uno o varios ficheros + campos de metadatos (`empresa`, `proyecto_id`, `tipo_doc`, `idioma`). Ejecuta el pipeline completo (parse → elements → chunks → index) para cada fichero vía `asyncio.to_thread()` (Docling es bloqueante). Devuelve `{"ok": true, "results": [{file, children, parents} | {file, error}]}`.
